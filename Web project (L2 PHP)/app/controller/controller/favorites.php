...
function main_favoris(): string
{
    ...
    // Affichage
    $ids = favorites_get();
    $html = '<p>Total : ' . count($ids) . ' article(s) favori(s)</p>';
    $html .= '<form method="POST"><button name="action" value="clear">🗑️ Vider les favoris</button></form>';
    ...
    if (!empty($ids)) {
        ...
        $html .= '<p>Aucun favori pour le moment.</p>';
    }
